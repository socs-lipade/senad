...
implements IOFSwitchListener, IFloodlightModule, IStaticEntryPusherService, IStorageSourceListener, IOFMessageListener {
	...
	private Map<String, Map<String, OFMessage>> readEntriesFromStorage() {
		...
		System.out.println("read all entris readEntriesFromStorage():" + entries);
		return entries;
	}
	...
	public void rowsModified(String tableName, Set<Object> rowKeys)  {
		...
		FlowEntryProducer flowEtryProducer = new FlowEntryProducer();
		Thread thr = new Thread(flowEtryProducer);
		...
		for (Object key: rowKeys) {
			...
			while (it.hasNext()) {
				...
				//Map<String, String> map = Splitter.on(", ").withKeyValueSeparator("=").split(row.toString());
				//map.put(String.valueOf( System.currentTimeMillis()), String.valueOf(row));
				System.out.println("*****row insert and update rowsModified(): " + "; row:"+ row);
				...
				System.out.println("*****row insert and update rowsModified(): " + "; row.get(actions):"+ row.get("actions"));
				
				dataValueString = "{\"switch\":"+"\"" +row.get("switch")+"\","
				+"\"in_port\":"+"\"" +row.get("in_port")+"\"" + ","
				+"\"ipv4_src\":"+"\"" +row.get("ipv4_src")+"\"" + ","
				+"\"ipv4_dst\":"+"\"" +row.get("ipv4_dst")+"\"" + ","
				+"\"eth_src\":"+"\"" +row.get("eth_src")+"\"" + ","
				+"\"eth_dst\":"+"\"" +row.get("eth_dst")+"\"" + ","
				+"\"eth_type\":"+"\"" +row.get("eth_type")+"\"" + ","				
				+"\"priority\":"+ "\"" + row.get("priority") + "\"" + ","								
				+"\"actions\":" +"\"" +row.get("actions").toString().replaceAll("=", "0x3D") +"\""				
				+"}";
				
				myData.put(row.get("name").toString(), dataValueString);
				
						
				flowEtryProducer.sendMsg(myData);
		...
		try {
			//Map<String, Object> myDataString = StaticEntries.jsonToStorageEntry(entriesToAdd.toString());
			//myData.put(String.valueOf(System.currentTimeMillis()), myDataString.toString());
			//System.out.println("*****row insert and update rowsModified(): " + "; entriesToAdd:"+ myDataString);
	...
	private void deleteStaticFlowEntry(String entryName) {
		...
		System.out.println("*** flow deleted:"+entryName);
	...
	public Map<String, Map<String, OFMessage>> getEntries() {
		...
		System.out.println("***** getEntries():" + entriesFromStorage);
